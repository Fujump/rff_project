...
def get_rff_matrix(dim_in, dim_out, std):
    ...
    return std * np.hstack((np.random.randn(dim_in, int(1/2*dim_out)), (np.random.randn(dim_in, int(1/2*dim_out))+0.1)))
    # return std * np.hstack((np.random.randn(dim_in, dim_out), (np.random.randn(dim_in, dim_out)+0.1)))
